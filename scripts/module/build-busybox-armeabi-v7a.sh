...
# Build the bundled Android armeabi-v7a BusyBox helper from official BusyBox sources.
...
OUT_PATH="${1:-$ROOT_DIR/prebuilt/bin/armeabi-v7a/busybox}"
...
BUILD_DIR="$WORK_DIR/build/armeabi-v7a"
...
build_busybox() {
  ...
  cc="$tc/armv7a-linux-androideabi${ANDROID_API}-clang"
  ar="$tc/llvm-ar"
  ranlib="$tc/llvm-ranlib"
  strip="$tc/llvm-strip"
  [[ -x "$cc" ]] || fail "Android NDK clang not found for armeabi-v7a: $cc"

  rm -rf "$BUILD_DIR"
  configure_busybox

  msg "building BusyBox $BUSYBOX_VERSION for armeabi-v7a with $cc"
  ...
  if command -v file >/dev/null 2>&1; then
    file "$OUT_PATH" | grep -qiE 'ARM.*32-bit|ELF 32-bit.*ARM' || fail "BusyBox output is not armv7 ELF: $(file "$OUT_PATH")"
  fi
